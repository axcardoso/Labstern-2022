...
def hit_or_miss(n = 4900000, Seed = None):
    random.seed(Seed)
    """
    Esta funcao deve retornar a sua estimativa para o valor da integral de f(x)
    ...
    return ponto_dentro/n


def control_variate(n = 120000, Seed = None):
    random.seed(Seed)
    """
    Esta funcao deve retornar a sua estimativa para o valor da integral de f(x)
...
def plot_hist():
    """
    função para plotar o histogramas de 1000 estimativas dos métodos de Monte-Carlo
    
    """
    lista = []
    for i in range(1000):
        lista += [control_variate()]
        print(f'O progresso está {round((i/1000)*100,2)}% completo')
